...
import {
  cleanWorkoutContent,
  isRecommendationContent,
} from "../utils/test-parser";
...
const MissingProfileFieldsForm = ({
  missingFields,
  currentProfileData,
  onSave,
  onGenerate,
  ...
  const focusOnFirstMissingField = () => {
    ...
    if (!formData.activityLevel) {
      ...
      if (activityLevelRef.current) {
        ...
        activityLevelRef.current.style.boxShadow =
  ...
  const handleInputChange = (field, value) => {
    setFormData((prev) => {
      ...
      if (field === "targetWeight" || field === "targetWeightUnit") {
        const targetWeight =
          field === "targetWeight" ? value : newFormData.targetWeight;
        const unit =
          field === "targetWeightUnit" ? value : newFormData.targetWeightUnit;
      ...
      if (
        ...
        field === "targetWeight" ||
        field === "weightUnit" ||
        field === "targetWeightUnit"
      ) {
        const currentWeight = field === "weight" ? value : newFormData.weight;
        const targetWeight =
          field === "targetWeight" ? value : newFormData.targetWeight;
        const currentUnit =
          field === "weightUnit" ? value : newFormData.weightUnit;
        const targetUnit =
          field === "targetWeightUnit" ? value : newFormData.targetWeightUnit;
  ...
  const validateWeights = (
    ...
    if (currentUnit && targetUnit && currentUnit !== targetUnit) {
      setWeightError(
        "Current weight and target weight should use the same units",
      );
      return;
    }
      ...
      >
        ...
        >
          ...
          >
            ...
            {isFieldRequired("targetTimeline") && (
              <span style={{ color: "#ef4444", fontWeight: "600" }}> *</span>
            )}
          </label>
          <input
        ...
        <button
          ...
          onClick={() => {
            ...
            if (!formData.age) {
              ...
              if (ageRef.current) {
                ageRef.current.style.border = "2px solid #ef4444";
                ageRef.current.style.boxShadow =
                  "0 0 0 3px rgba(239, 68, 68, 0.3)";
                setTimeout(() => {
                  if (ageRef.current) {
                    ageRef.current.style.border = ageError
                      ? "1px solid #ef4444"
                      : "1px solid #d1d5db";
                    ageRef.current.style.boxShadow = "none";
                  }
            ...
            if (!formData.gender) {
              ...
              if (genderRef.current) {
                genderRef.current.style.border = "2px solid #ef4444";
                genderRef.current.style.boxShadow =
                  "0 0 0 3px rgba(239, 68, 68, 0.3)";
                setTimeout(() => {
                  if (genderRef.current) {
                    genderRef.current.style.border = "1px solid #d1d5db";
...
const AIFitnessCoach = () => {
  ...
  const [showMissingProfileFieldsPopup, setShowMissingProfileFieldsPopup] =
    useState(false);
  ...
  const [openSuggestionsAccordion, setOpenSuggestionsAccordion] =
    useState(false);
  ...
  const [showWorkoutDaysConfirmation, setShowWorkoutDaysConfirmation] =
    useState(false);
  ...
  const [profileCurrentWeightError, setProfileCurrentWeightError] =
    useState("");
        ...
        meals: [
          ...
          },
          {
            ...
            items: [
              ...
              {
                description: "Side salad with olive oil dressing",
                calories: 150,
              },
            ],
          },
      ...
      try {
        ...
        if (verifiedSession) {
          try {
            ...
            console.log(
              "🔐 DEBUG: Found verified customer session data:",
              sessionData,
            );
            ...
            if (sessionAge < maxAge && sessionData.verified) {
              console.log(
                "✅ DEBUG: Valid verified customer session found, auto-logging in...",
              );
              ...
              console.log(
                "⏰ DEBUG: Verified customer session expired, clearing...",
              );
              localStorage.removeItem("verifiedCustomerSession");
            }
          } catch (error) {
            console.error(
              "❌ DEBUG: Error parsing verified customer session:",
              error,
            );
            localStorage.removeItem("verifiedCustomerSession");
          }
        ...
        if (sessionTransfer === "true" && email && customerId) {
          console.log(
            "🛍️ DEBUG: Session transfer detected, AuthGuard will handle redirect",
          );
          return;
        }
        ...
        if (token) {
          console.log(
            "🤖 AiCoach: Token found, redirecting to auth page for processing...",
          );
        ...
        console.log(
          "🤖 AiCoach: No special parameters, letting AuthGuard handle authentication",
        );
      } catch (error) {
  ...
  const goalData = {
    "Lose Weight": {
      ...
      color: "#e74c3c",
      description:
        "Focus on calorie deficit and cardio exercises to achieve sustainable weight loss.",
    },
    "Build Muscle": {
      icon: "fa-dumbbell",
      color: "#3498db",
      description:
        "Combine strength training with proper nutrition to increase muscle mass.",
    },
    "Get Fit": {
      icon: "fa-running",
      color: "#2ecc71",
      description:
        "Enhance your cardiovascular fitness through consistent aerobic exercises.",
    },
    "Get Stronger": {
      icon: "fa-mountain",
      color: "#f1c40f",
      description:
        "Focus on progressive overload and compound movements for overall strength.",
    },
    "Stay Flexible": {
      icon: "fa-stopwatch",
      color: "#9b59b6",
      description:
        "Incorporate stretching and mobility exercises for better range of motion.",
    },
    "Be Athletic": {
      icon: "fa-chart-line",
      color: "#1abc9c",
      description:
        "Enhance your overall athletic abilities through targeted training programs.",
    },
  };
  ...
  const formatResponse = (text) => {
    ...
    cleaned = cleaned.replace(
      /MEAL_PLAN(?:\s+for\s+[\w\s]+)?:/gi,
      "MEAL_PLAN:",
    );
    cleaned = cleaned.replace(
      /WORKOUT_PLAN(?:\s+for\s+[\w\s]+)?:/gi,
      "WORKOUT_PLAN:",
    );
    ...
    cleaned = cleaned.replace(
      /\*\*Total Daily Calories:\*\*\s*(\d+)/gi,
      "Total Daily Calories: $1",
    );
    cleaned = cleaned.replace(
      /\*\*Timeline:\*\*\s*(\d+)\s*months?/gi,
      "Timeline: $1 months",
    );
    cleaned = cleaned.replace(
      /\*\*Weekly Schedule:\*\*\s*(\d+)\s*days?/gi,
      "Weekly Schedule: $1 days",
    );
    cleaned = cleaned.replace(
      /\*\*Expected Results:\*\*\s*/gi,
      "Expected Results: ",
    );
    cleaned = cleaned.replace(
      /\*\*Day\s*(\d+)\s*-\s*([^:]+):\*\*/gi,
  ...
  const saveConversationData = async (
    ...
    console.log("saveConversationData called with:", {
      prompt,
      response,
      requestParams,
      metadata,
    ...
    try {
      ...
      const conversationMetadata = {
        ...
        planData: {
          ...
          mealPlanContent: response.includes("MEAL_PLAN:")
            ? extractPlanContent(response, "MEAL_PLAN:")
            : null,
          workoutPlanContent: response.includes("WORKOUT_PLAN:")
            ? extractPlanContent(response, "WORKOUT_PLAN:")
            : null,
          // Add structured workout plan data from component state
          structuredWorkoutPlan:
            workoutSections && workoutSections.length > 0
              ? workoutSections
              : null,
        },
      ...
      console.log(
        "Final conversation metadata to be saved:",
      ...
      console.log(
        "Structured conversation data saved successfully with ID:",
        conversationId,
      );
  ...
  const extractPlanContent = (response, planType) => {
    ...
    const nextPlanStart = response.indexOf(
      "WORKOUT_PLAN:",
      startIndex + planType.length,
    );
  ...
  const extractProfileData = (prompt) => {
    ...
    if (
      parsedData.gender &&
      ["male", "female", "other"].includes(parsedData.gender.toLowerCase())
    ) {
  ...
  const validateProfileData = (extractedData, originalPrompt) => {
    ...
    if (!extractedData.age) {
      ...
      suggestions.push(
        'Please include your age (e.g., "I am 25 years old" or "25M")',
      );
    }
    ...
    if (!extractedData.gender) {
      ...
      suggestions.push(
        'Please specify your gender (e.g., "male", "female", "M", "F")',
      );
    }
    ...
    if (!extractedData.height) {
      ...
      suggestions.push(
        'Please include your height (e.g., "5\'8", "170 cm", "5 feet 8 inches")',
      );
    }
    ...
    if (!extractedData.weight) {
      ...
      suggestions.push(
        'Please include your weight (e.g., "70 kg", "150 lbs", "70kg")',
      );
    }
    ...
    if (!extractedData.activityLevel) {
      ...
      suggestions.push(
        'Please specify your activity level (e.g., "sedentary", "moderately active", "very active")',
      );
    }
  ...
  useEffect(() => {
    const saveCombinedConversation = async () => {
      if (
        combinedConversationData.isMealPlanComplete &&
        combinedConversationData.isWorkoutPlanComplete
      ) {
  ...
  const generatePlanWithProfileChoice = async (useProfile, extractedData) => {
    try {
      ...
      if (Object.keys(extractedData).length > 0 && user) {
        ...
        const userDoc = await getDoc(userRef);
        const hasProfile =
          userDoc.exists() && userDoc.data().height && userDoc.data().weight;

        if (hasProfile) {
          // Always compare extracted data with existing profile, regardless of useProfile choice
          const userData = userDoc.data();
          console.log("Comparing extracted data with profile:", {
            extractedData,
            userData,
          });
          ...
          for (const key of Object.keys(extractedData)) {
            if (
              ...
              if (profileValue === undefined || profileValue === null) {
                ...
                console.log(
                  `Field ${key} is new (not in profile):`,
                  extractedValue,
                );
                continue;
              }
              ...
              if (key === "height" || key === "weight" || key === "age") {
                ...
                if (key === "weight") {
                  if (
                    Math.abs(extractedNum - profileNum) > 0.1 &&
                    extractedNum > 0
                  ) {
                    changedFields[key] = extractedNum;
                    console.log(
                      `Weight changed: ${profileNum} -> ${extractedNum}`,
                    );
                  }
                } else {
                  ...
                  if (extractedInt !== profileInt && extractedInt > 0) {
                    ...
                    console.log(
                      `${key} changed: ${profileInt} -> ${extractedInt}`,
                    );
                  }
                }
              } else if (key === "gender") {
                ...
                if (
                  extractedGender !== profileGender &&
                  extractedGender !== ""
                ) {
                  changedFields[key] = extractedValue;
                  console.log(
                    `Gender changed: ${profileGender} -> ${extractedGender}`,
                  );
                }
                ...
                if (
                  extractedActivity !== profileActivity &&
                  extractedActivity !== "moderate"
                ) {
                  changedFields[key] = extractedValue;
                  console.log(
                    `Activity level changed: ${profileActivity} -> ${extractedActivity}`,
                  );
                }
              } else if (extractedValue !== profileValue) {
                changedFields[key] = extractedValue;
                console.log(
                  `${key} changed: ${profileValue} -> ${extractedValue}`,
                );
              }
            }
          }
        ...
        if (Object.keys(changedFields).length > 0) {
          console.log(
            "Showing profile update popup with changed fields:",
            changedFields,
          );
  ...
  const handleRecommendation = async () => {
    ...
    try {
      ...
      const { hasProfile, hasCompleteProfile, profileData } =
      ...
      if (
        ...
        if (hasCompleteProfile) {
          ...
          console.log(
            "Processing with extracted data from prompt:",
            extractedData,
          );
  ...
  const handleUseProfileChoice = async (useProfile) => {
    ...
    if (useProfile) {
      ...
      console.log(
        "User selected No to use profile, processing prompt directly:",
        fitnessGoal,
      );
        ...
        try {
            ...
            body: JSON.stringify({
              ...
              userDetails: {
                ...
                targetWeight: parseFloat(
                  extractedData.targetWeight?.value || extractedData.weight,
                ),
                timelineWeeks: parseInt(
                  extractedData.timelineWeeks ||
                    extractedData.timeline?.value ||
                    12,
                ),
          ...
          const calorieRequestParams = {
            ...
            userDetails: {
              ...
              targetWeight: parseFloat(
                extractedData.targetWeight?.value || extractedData.targetWeight,
              ),
          ...
          await saveConversationData(
            fitnessGoal,
            JSON.stringify(calorieData),
            calorieRequestParams,
            {
      ...
      if (user) {
        ...
        if (userDoc.exists()) {
          ...
          setProfileFormData({
            ...
            targetWeightUnit:
              userData.targetWeightUnit || userData.weightUnit || "kg",
            activityLevel: userData.activityLevel || "",
  ...
  const handleProfileFormSubmit = async (e) => {
    ...
    validateProfileCurrentWeight(
      profileFormData.currentWeight,
      profileFormData.currentWeightUnit,
    );
    validateProfileTargetWeight(
      profileFormData.targetWeight,
      profileFormData.targetWeightUnit,
    );
    ...
    if (
      ...
      !profileFormData.targetTimeline
    ) {
  ...
  const handleMealPlanChoice = async (generateMealPlan) => {
    ...
    const mealStreamingTimeout = setTimeout(() => {
      console.log(
        "Meal plan streaming timeout reached, forcing isStreamingMeal to false",
      );
      setIsStreamingMeal(false);
    ...
    try {
      const response = await fetch(getApiUrl("mealplan"), {
        ...
        body: JSON.stringify({
          ...
          dietaryRestrictions: useExistingProfile
            ? userProfileData?.dietaryRestrictions || []
            : [], // Only send if user chose to use profile
          allergies: useExistingProfile ? userProfileData?.allergies || [] : [], // Only send if user chose to use profile
          healthGoals:
            extractedProfileData?.healthGoals || profileFormData?.goal,
          targetWeight:
            extractedProfileData?.targetWeight?.value ||
            profileFormData?.targetWeight,
          timelineWeeks: parseInt(
            extractedProfileData?.timelineWeeks ||
              profileFormData?.targetTimeline * 4 ||
              12,
          ),
          prompt:
            fitnessGoal ||
            `Generate meal plan for ${extractedProfileData?.goal || profileFormData.goal || "fitness"} goal`,
        }),
      });
      ...
      while (true) {
        ...
        while (dayMatches.length >= 2) {
          ...
          const completeDayText = currentDayBuffer.substring(
            firstDayStart,
            secondDayStart,
          );
          ...
          try {
            ...
            if (dayNumberMatch) {
              ...
              if (
                dayNumber >= 1 &&
                dayNumber <= 7 &&
                !processedDayNumbers.has(dayNumber)
              ) {
                ...
                console.log(
                  `Completed day ${dayNumber} of meal plan (${completedDays} total processed)`,
                );
                if (completedDays >= 7) {
                  console.log(
                    `DEBUG: currentMealDay set to ${completedDays}, will show "Finalizing meal plan..."`,
                  );
                } else {
                  console.log(
                    `DEBUG: currentMealDay set to ${completedDays}, will show "Generating Day ${completedDays + 1} of 7..."`,
                  );
                }
                console.log(
                  `DEBUG: Progress will show "${completedDays} of 7 days completed"`,
                );
              }
            }
          } catch (parseError) {
          ...
          const remainingDayMatches = [
            ...currentDayBuffer.matchAll(dayPattern),
          ];
        ...
        if (dayMatches.length === 1 && completedDays < 7) {
          ...
          const suggestionIndex =
            currentDayBuffer.search(/END-OF-PLAN[\s-]?SUGGESTION/i) !== -1
          ...
          const totalCaloriesIndex = currentDayBuffer.indexOf(
          ...
          if (totalCaloriesIndex > dayStart) {
            ...
            const completeDayText = currentDayBuffer.substring(
              dayStart,
              totalCaloriesIndex + 50,
            ); // Include some extra characters
            ...
            try {
              ...
              if (dayNumberMatch) {
                ...
                if (
                  ...
                  setCurrentMealDay(completedDays);
                  console.log(
                    `Completed final day ${dayNumber} of meal plan (${completedDays} total processed)`,
                  );
                  if (completedDays >= 7) {
                    console.log(
                      `DEBUG: currentMealDay set to ${completedDays}, will show "Finalizing meal plan..."`,
            ...
            console.log(
              "DEBUG: Suggestion text to parse:",
              suggestionText.substring(0, 200) + "...",
            );
            const suggestionMatch =
              suggestionText.match(/END-OF-PLAN[\s-]?SUGGESTION:?\s*(.*)/i) ||
        ...
        try {
          ...
          console.log(
            "Parsing meal response:",
            liveResponse.substring(0, 200) + "...",
          );
          ...
          console.log(
            "Temporary meal parse error (continuing):",
            parseError.message,
          );
      ...
      console.log(
        "Meal plan generation completed, setting isStreamingMeal to false",
      );
  ...
  const handleWorkoutPlanChoice = async (generateWorkoutPlan) => {
    ...
    if (
      ...
      (extractedProfileData.workoutDays || extractedProfileData.frequency)
    ) {
      const extractedDays = parseInt(
        extractedProfileData.workoutDays || extractedProfileData.frequency,
      );
  ...
  const generateWorkoutPlanWithDays = async (workoutDays) => {
    ...
    const streamingTimeout = setTimeout(() => {
      console.log(
        "Streaming timeout reached, forcing isStreamingWorkout to false",
      );
    ...
    try {
      ...
      workoutPrompt =
        fitnessGoal ||
        `Generate workout plan for ${extractedProfileData?.goal || profileFormData.goal || "fitness"} goal, I prefer to workout ${workoutDays} days per week`;

      console.log("Final workout prompt:", workoutPrompt);
      console.log("Sending to backend:", {
        goal: extractedProfileData?.goal || profileFormData.goal || "fitness",
        workout_focus:
          calculatedCalories.WorkoutFocus || "Mixed Cardio and Strength",
      ...
      while (true) {
        ...
        try {
          ...
          console.log(
            "Current workout sections after parsing:",
            workoutSections,
          );
        } catch (parseError) {
          console.log(
            "Temporary workout parse error (continuing):",
            parseError.message,
          );
          // Fallback: try to create days from text
        ...
        if (specificDayMatches) {
          const specificDays = specificDayMatches
            .map((match) => {
          ...
          setWorkoutSections((prevSections) => {
            ...
            specificDays.forEach((dayNum) => {
              if (!updatedSections.find((day) => day.dayNumber === dayNum)) {
                console.log(
                  `Creating workout day ${dayNum} from specific pattern during streaming`,
                );
      ...
      */
      ...
      if (workoutSuggestionMatch) {
        ...
        console.log(
          "DEBUG: Extracted workout plan suggestion:",
          extractedWorkoutSuggestion,
        );
        ...
        console.log(
          "DEBUG: Set workout plan suggestion:",
          extractedWorkoutSuggestion,
        );
      ...
      console.log(
        "Workout plan generation completed, setting isStreamingWorkout to false",
      );
  ...
  const generateSuggestions = async (workoutPlanText = "") => {
    try {
      ...
      console.log(
        ...
        endOfPlanSuggestion || "fallback suggestions",
      );
  ...
  const generateFallbackSuggestions = () => {
    ...
    const userWeight =
      profileFormData.currentWeight || extractedProfileData?.weight || "";
    const targetWeight =
      profileFormData.targetWeight ||
      extractedProfileData?.targetWeight?.value ||
      userWeight;
    const activityLevel =
      profileFormData.activityLevel ||
      extractedProfileData?.activityLevel ||
      "";
    const workoutDays =
      profileFormData.workoutDays || extractedProfileData?.workoutDays || "5";
    ...
    if (hasMealPlan && hasWorkoutPlan) {
      suggestions = [
        ...
        userWeight && targetWeight && userWeight !== targetWeight
          ? `• Track your progress from ${userWeight}kg towards your target of ${targetWeight}kg`
          : "",
        ...
        userAge
          ? `• At ${userAge} years old, focus on proper recovery between workout sessions`
          : "• Focus on proper recovery between workout sessions",
      ...
      suggestions = [
        ...
        userWeight && targetWeight && userWeight !== targetWeight
          ? `• Monitor your progress from ${userWeight}kg towards ${targetWeight}kg through nutrition`
          : "",
        ...
        userAge
          ? `• At ${userAge} years old, proper nutrition is especially important for achieving your goals`
          : "",
      ...
      suggestions = [
        ...
        userWeight && targetWeight && userWeight !== targetWeight
          ? `• Work towards your target of ${targetWeight}kg from ${userWeight}kg through consistent training`
          : "",
        `• Consider adding a structured nutrition plan to complement your workout routine`,
        userAge
          ? `• At ${userAge} years old, ensure adequate rest between your workout sessions`
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        const currentAge = parseInt(
          profileFormData.age || extractedProfileData?.age,
        );
        const currentGender =
          profileFormData.gender || extractedProfileData?.gender;
        const currentHeight = parseInt(
          profileFormData.height || extractedProfileData?.height,
        );
        const currentWeight = parseFloat(
          profileFormData.currentWeight || extractedProfileData?.weight,
        );
        const currentActivityLevel =
          profileFormData.activityLevel || extractedProfileData?.activityLevel;
        ...
        if (
          currentHeight &&
          parseInt(currentHeight) !== parseInt(userData.height)
        ) {
          changedFields.height = parseInt(currentHeight);
        }
        if (
          currentWeight &&
          parseFloat(currentWeight) !== parseFloat(userData.weight)
        ) {
  ...
  const handleSaveProfile = async (formData) => {
    ...
    try {
      ...
      const {
        ...
        targetWeightUnit: targetWU,
        targetTimeline: targetT,
        targetTimelineUnit: targetTU,
        workoutDays: workoutD,
        ...dataToSave
      } = formData;
      ...
      if (validationErrors.length > 0) {
        ...
        setTimeout(() => {
          if (!formData.age) {
            ...
            const targetWeightInput = document.getElementById(
              "profileTargetWeight",
            );
            ...
            const workoutDaysSelect =
              document.getElementById("profileWorkoutDays");
  ...
  const parseResponseLive = (response, type = "meal") => {
    ...
    if (type === "workout") {
      ...
      if (currentLength - lastParsedLength < 30) {
        // Further reduced threshold for more frequent updates
  ...
  const parseResponseLiveImmediate = (response, type = "meal") => {
    if (type === "meal") {
      ...
      const mealPlanMatch = response.match(
        /MEAL_PLAN:([\s\S]*?)(?=WORKOUT_PLAN:|END-OF-PLAN[\s-]?SUGGESTION:|\*\*End-of-Plan Suggestion:\*\*|\*\*Recommendation:\*\*|$)/i,
      );
      ...
      const mealPlanLines = mealPlanRaw
        .split("\n")
        .map((line) => line.trim())
        .filter(Boolean);
      let currentDay = null;
        ...
        mealPlanLines.forEach((line) => {
          ...
          if (dayMatch) {
            ...
            if (dayNumber >= 1 && dayNumber <= 7) {
              ...
              console.log(
                `DEBUG: Skipping Day ${dayNumber} (not in range 1-7)`,
              );
            }
          ...
          const mealTypeMatch = line.match(
            /(Breakfast|Lunch|Snack|Dinner)\s*\((\d+)\s*(?:kcal|calories?)?\)(?:\s*:|$)/i,
          );
          if (mealTypeMatch && currentDay && currentDay <= 7) {
            currentMealType = mealTypeMatch[1];
            const calories = parseInt(mealTypeMatch[2]) || 0;
            console.log(
              `DEBUG: Found meal type: ${currentMealType} with ${calories} kcal for Day ${currentDay}`,
            );
            console.log(
              `DEBUG: Setting meal totalCalories to ${calories} for ${currentMealType}`,
            );

            const dayData = updatedMealPlans.find(
              (day) => day.dayNumber === currentDay,
            );
            if (dayData) {
              const mealIndex = dayData.meals.findIndex(
                (m) => m.type === currentMealType,
              );
              if (mealIndex !== -1) {
                ...
                console.log(
                  `DEBUG: Set ${currentMealType} totalCalories to ${calories} for Day ${currentDay}`,
                );
                hasChanges = true;
              }
            }
          ...
          const itemMatch = line.match(
            /^\s*\d+\.\s*(.*?)\s*[-–—]\s*(.*?)\s*[-–—]\s*(\d+)\s*(?:kcal|calories?|cal)\s*$/i,
          );
          if (itemMatch && currentDay && currentMealType && currentDay <= 7) {
            ...
            console.log(
              "DEBUG: Raw calories text:",
              itemMatch[3],
              "Parsed calories:",
              itemCalories,
            );
            console.log(
              "DEBUG: Current meal type:",
              currentMealType,
              "Current day:",
              currentDay,
            );
            ...
            if (dayData) {
              ...
              if (mealIndex !== -1) {
                ...
                const existingItem = dayData.meals[mealIndex].items.find(
                  (item) =>
          ...
          const totalCaloriesMatch = line.match(
            /Total Daily Calories:\s*(\d+)\s*(?:kcal|calories?|cal)/i,
          );
          if (totalCaloriesMatch && currentDay && currentDay <= 7) {
            ...
            console.log(
              `Found total calories for day ${currentDay}: ${totalCaloriesMatch[1]}`,
            );
          }
        });
      ...
      const workoutPlanMatch = response.match(
        /WORKOUT_PLAN:([\s\S]*?)(?=\b(?:END-OF-PLAN[\s-]?SUGGESTION|END-OF-PLAN SUGG|END-OF-PLAN|END-OF|END OF)\b[:]?|\b(?:Follow this plan consistently|Closing Recommendation|To achieve your goal|To effectively)\b[:]?|\b(?:RECOMMENDATION|SUGGESTION|CLOSING|FINAL NOTE|FINAL SUGGESTION|FINAL RECOMMENDATION)\b[:]|$)/i,
      );
      ...
      const workoutPlanLines = workoutPlanRaw
        .split("\n")
        .map((line) => line.trim())
        .filter(Boolean);
      ...
      setWorkoutSections((prevWorkoutSections) => {
        ...
        dayNumbers.forEach((dayNum) => {
          if (!updatedWorkoutSections.find((day) => day.dayNumber === dayNum)) {
            ...
            console.log(
              `Creating workout day ${dayNum} immediately: ${workoutType}`,
            );
        ...
        const filteredWorkoutLines = workoutPlanLines.filter(
          (line) => !isRecommendationContent(line),
        );

        filteredWorkoutLines.forEach((line) => {
          // Skip if this line is just "END-OF" or similar
          if (
            line.trim().toUpperCase() === "END-OF" ||
            line.trim().toUpperCase() === "END OF"
          ) {
          ...
          const exclusionMatch = line.match(
            /Day\s*(\d+)[:,]\s*(\d+),\s*and\s*(\d+)\s*are\s*not\s*included/i,
          );
          if (exclusionMatch) {
            ...
            [day1, day2, day3].forEach((dayNum) => {
              if (dayNum >= 1) {
                const existingDay = updatedWorkoutSections.find(
                  (day) => day.dayNumber === dayNum,
                );
                if (existingDay) {
                ...
                hasChanges = true;
                console.log(
                  `Set Day ${dayNum} as Rest Day due to exclusion message (live)`,
                );
              }
            });
            return;
          ...
          const singleExclusionMatch = line.match(
            /Day\s*(\d+)[:\s]*not\s*included/i,
          );
          if (singleExclusionMatch) {
            ...
            if (dayNum >= 1) {
              ...
              hasChanges = true;
              console.log(
                `Set Day ${dayNum} as Rest Day due to single exclusion message (live)`,
              );
            }
            return;
          }
          ...
          if (!dayMatch) {
            dayMatch =
              line.match(/Day\s*(\d+)[:\s–-]/i) || line.match(/Day\s*(\d+)$/i);
          }

          if (dayMatch) {
            currentWorkoutDay = parseInt(dayMatch[1]);
            currentWorkoutType = dayMatch[2]
              ? dayMatch[2].trim()
              : "Training Day";

            console.log(
              `Found workout day ${currentWorkoutDay} with type: "${currentWorkoutType}"`,
            );
            ...
            if (currentWorkoutDay > 0) {
              const existingDay = updatedWorkoutSections.find(
                (day) => day.dayNumber === currentWorkoutDay,
              );
              if (existingDay) {
                // Update existing day
                if (existingDay.workoutType !== currentWorkoutType) {
                  ...
                  hasChanges = true;
                  console.log(
                    `Updated workout day ${currentWorkoutDay}: ${currentWorkoutType}`,
                  );
                }
              } else {
                // Create new day
                console.log(
                  `Creating workout day ${currentWorkoutDay}: ${currentWorkoutType}`,
                );
                updatedWorkoutSections.push({
          ...
          if (
            ...
            if (dayData && dayData.exercises.length > 0) {
              ...
              if (
                ...
                hasChanges = true;
                console.log(
                  `Appended continuation to exercise ${lastExercise.number}: ${currentLine}`,
                );
                return;
              }
            }
          ...
          if (currentWorkoutDay && exerciseNumberMatch) {
            ...
            if (
              exerciseText.toLowerCase().includes("day") &&
              exerciseText.length < 20
            ) {
            ...
            if (
              exerciseText.match(/^END[-–—]?$/i) ||
              exerciseText.match(/^END\s*$/i)
            ) {
            ...
            if (
              exerciseText.match(/^Follow(\s+this)?\s*$/i) ||
            ...
            if (dayData) {
              ...
              if (
                isRecommendationContent(exerciseText) ||
                exerciseText.trim().toUpperCase() === "END-OF"
              ) {
                console.log(
                  `Skipping END-OF/recommendation content: "${exerciseText}"`,
                );
                return;
              }

              const existingExercise = dayData.exercises.find(
                (ex) => ex.number === exerciseNumber,
              );
              ...
              if (!existingExercise) {
                ...
                hasChanges = true;
                console.log(
                  `Added NEW exercise ${exerciseNumber} to day ${currentWorkoutDay}: ${exerciseText}`,
                );
              } else if (
                exerciseText.length > existingExercise.description.length
              ) {
                // Update existing exercise only if the new one is significantly more complete
                existingExercise.description = exerciseText;
                hasChanges = true;
                console.log(
                  `Updated exercise ${exerciseNumber} to more complete version: ${exerciseText}`,
                );
              } else {
                console.log(
                  `Skipping duplicate/incomplete exercise ${exerciseNumber}: ${exerciseText}`,
                );
              }
            }
            return;
          ...
          if (
            ...
            if (
              exercise.toLowerCase().includes("day") &&
              exercise.length < 20
            ) {
            ...
            // Skip suggestion content like "Follow", "Follow this", etc.
            if (
              exercise.match(/^Follow(\s+this)?\s*$/i) ||
            ...
            if (isRecommendationContent(exercise)) {
              console.log(
                `Skipping recommendation content in exercise: "${exercise}"`,
              );
              return;
            }
            ...
            if (dayData) {
              // Skip if this is END-OF or recommendation content
              if (
                isRecommendationContent(exercise) ||
                exercise.trim().toUpperCase() === "END-OF"
              ) {
                console.log(
                  `Skipping END-OF/recommendation content: "${exercise}"`,
                );
                return;
              }
              ...
              if (!existingExercise) {
                ...
                hasChanges = true;
                console.log(
                  `Added plain text exercise to day ${currentWorkoutDay}: ${exercise}`,
                );
              } else {
  ...
  const parseOriginalResponse = (originalResponse) => {
    ...
    const mealPlanMatch = originalResponse.match(
      /MEAL_PLAN:([\s\S]*?)(?=WORKOUT_PLAN:|$)/i,
    );
    if (!mealPlanMatch) return;
    ...
    mealPlanLines.forEach((line, index) => {
      ...
      if (dayMatch) {
        ...
        if (dayNumber >= 1 && dayNumber <= 7) {
          ...
          console.log(
            `DEBUG: Skipping Day ${dayNumber} (not in range 1-7) in original response`,
          );
        }
        return;
      }
      ...
      const mealTypeMatch = line.match(
        /(Breakfast|Lunch|Snack|Dinner)\s*\((\d+)\s*kcal\):/i,
      );
      if (mealTypeMatch && currentDay && currentDay <= 7) {
        currentMealType = mealTypeMatch[1];
        ...
        let dayData = updatedMealPlans.find(
          (day) => day.dayNumber === currentDay,
        );
        if (!dayData) {
        ...
        console.log(
          `DEBUG: Set ${currentMealType} total calories to ${mealCalories} for Day ${currentDay}`,
        );
      ...
      if (itemMatch && currentDay && currentMealType && currentDay <= 7) {
        ...
        if (caloriesText.includes("+")) {
          ...
          const calorieParts = caloriesText
            .split("+")
        ...
        if (dayData) {
          ...
          if (mealIndex !== -1) {
            ...
            if (!existingItem) {
              ...
              console.log(
                `DEBUG: Added item to ${currentMealType}:`,
                description,
                itemCalories,
                "kcal",
              );
              ...
              console.log(
                `DEBUG: Item already exists in ${currentMealType}:`,
                description,
                itemCalories,
                "kcal",
              );
    ...
    updatedMealPlans.forEach((day) => {
      ...
      day.meals.forEach((meal) => {
        console.log(
          `  ${meal.type}: ${meal.totalCalories} kcal (${meal.items.length} items)`,
        );
      });
    });
  };
  ...
  const parseResponse = (response) => {
    ...
    if (!suggestionMatch) {
      ...
      suggestionMatch = cleanedResponse.match(
        /\*\*Closing Recommendation:\*\*([\s\S]*)$/i,
      );
    }
    if (!suggestionMatch) {
      suggestionMatch = cleanedResponse.match(
    ...
    if (!suggestionMatch) {
      ...
      suggestionMatch =
        cleanedResponse.match(/For[\s-]?your[\s-]?meal[\s-]?plan([\s\S]*)$/i) ||
        cleanedResponse.match(
          /Based[\s-]?on[\s-]?your[\s-]?profile([\s\S]*)$/i,
        ) ||
    ...
    if (suggestionMatch) {
      const extractedSuggestion = suggestionMatch[1].trim();
      console.log(
        "DEBUG: Extracted suggestion from backend:",
        extractedSuggestion,
      );
      setEndOfPlanSuggestion(extractedSuggestion);
      // Suggestions will be auto-opened by useEffect when first available
      console.log(
        "DEBUG: Set endOfPlanSuggestion from parseResponse and opened accordion",
      );
      console.log("DEBUG: Meal plan suggestion set to:", extractedSuggestion);
    } else {
      console.log("DEBUG: No suggestion found, using fallback");
      setEndOfPlanSuggestion(
        "Follow this plan consistently to achieve your desired results.",
      );
    }

    const mealPlanMatch = cleanedResponse.match(
      /MEAL_PLAN:([\s\S]*?)(?=WORKOUT_PLAN:|$)/i,
    );

    // Enhanced regex pattern to properly extract workout plan and stop before recommendations/suggestions
    let workoutPlanMatch = cleanedResponse.match(
      /WORKOUT_PLAN:([\s\S]*?)(?=\b(?:END-OF-PLAN[\s-]?SUGGESTION|END-OF-PLAN SUGG|END-OF-PLAN|END-OF|END OF)\b[:]?|\b(?:Follow this plan consistently|Closing Recommendation|To achieve your goal|To effectively)\b[:]?|\b(?:RECOMMENDATION|SUGGESTION|CLOSING|FINAL NOTE|FINAL SUGGESTION|FINAL RECOMMENDATION)\b[:]|$)/i,
    );

    if (!workoutPlanMatch) {
      console.log("Trying alternate workout plan pattern");
      workoutPlanMatch = cleanedResponse.match(
        /(?:WORKOUT|EXERCISE)[\s_]*PLAN:?([\s\S]*?)(?=\b(?:END-OF-PLAN[\s-]?SUGGESTION|END-OF-PLAN SUGG|END-OF-PLAN|END-OF|END OF)\b[:]?|\b(?:Follow this plan consistently|Closing Recommendation|To achieve your goal|To effectively)\b[:]?|\b(?:RECOMMENDATION|SUGGESTION|CLOSING|FINAL NOTE|FINAL SUGGESTION|FINAL RECOMMENDATION)\b[:]|$)/i,
      );
    }
    ...
    if (!workoutPlanMatch) {
      ...
      const workoutSectionMatch = cleanedResponse.match(
        /(WORKOUT_PLAN:|EXERCISE_PLAN:|###\s*Workout.*?)[\s\S]*?(?=\b(?:END-OF-PLAN[\s-]?SUGGESTION|END-OF-PLAN SUGG|END-OF-PLAN|END-OF|END OF)\b[:]?|\b(?:Follow this plan consistently|Closing Recommendation|To achieve your goal|To effectively)\b[:]?|\b(?:RECOMMENDATION|SUGGESTION|CLOSING|FINAL NOTE|FINAL SUGGESTION|FINAL RECOMMENDATION)\b[:]|$)/i,
      );
      if (workoutSectionMatch) {
        // Extract just the content after the header
        let content = workoutSectionMatch[0];
        const headerMatch = content.match(
          /^(WORKOUT_PLAN:|EXERCISE_PLAN:|###\s*Workout.*?)([\s\S]*)/i,
        );
        if (headerMatch) {
          workoutPlanMatch = [
            workoutSectionMatch[0],
            cleanWorkoutContent(headerMatch[2]),
          ];
        } else {
          workoutPlanMatch = [
            workoutSectionMatch[0],
            cleanWorkoutContent(workoutSectionMatch[0]),
          ];
        }
      }
    }
    ...
    if (!workoutPlanMatch && cleanedResponse.includes("###")) {
      ...
      for (let i = 0; i < sections.length; i++) {
        if (
          sections[i].toLowerCase().includes("workout") ||
    ...
    const workoutPlanRaw = workoutPlanMatch
      ? cleanWorkoutContent(workoutPlanMatch[1])
      : "";
    ...
    if (!workoutPlanRaw) {
      console.warn(
        "No workout plan section found - attempting to extract workout info from full response",
      );
      ...
      for (let i = 0; i < fullResponseLines.length; i++) {
        ...
        if (!foundWorkoutSection) {
          if (
            line.match(/work\s*out|exercise|training/i) &&
      ...
      if (workoutLines.length > 0) {
        console.log(
          "Found potential workout content:",
          workoutLines.join("\n"),
        );
        extractedWorkoutContent = cleanWorkoutContent(workoutLines.join("\n"));
        if (
          !extractedWorkoutContent.match(/day\s*\d+|exercise|workout|training/i)
        ) {
          console.log(
            "Extracted content doesn't look like workout content, using default",
          );
    ...
    const filteredWorkoutLines = extractedWorkoutContent
      .split("\n")
      .filter((line) => !isRecommendationContent(line));
    ...
    mealPlanLines.forEach((line) => {
      ...
      const mealTypeMatch = line.match(
        /(Breakfast|Lunch|Snack|Dinner)\s*(?:\((\d+)\s*(?:kcal|calories?)?\)|(?:\s*-\s*)?(\d+)\s*(?:kcal|calories?))?(?:\s*:|$)/i,
      );
      if (mealTypeMatch) {
        if (currentDay && currentMealType && currentMealItems.length > 0) {
          const dayIndex = currentDay - 1;
          if (dayIndex >= 0 && dayIndex < mealPlansByDay.length) {
            const mealIndex = mealPlansByDay[dayIndex].meals.findIndex(
              (m) => m.type === currentMealType,
            );
            if (mealIndex !== -1) {
              mealPlansByDay[dayIndex].meals[mealIndex].items =
                currentMealItems;
            }
          }
        }
        ...
        console.log(
          `Found meal type: ${currentMealType}, calories: ${calories}`,
        );
        ...
        if (currentDay && currentDay <= 7) {
          ...
          if (dayIndex >= 0 && dayIndex < mealPlansByDay.length) {
            ...
            if (mealIndex !== -1) {
              ...
              mealPlansByDay[dayIndex].meals[mealIndex].totalCalories =
                calories;
            }
          }
        }
    ...
    if (extractedWorkoutContent) {
      const workoutPlanLines = extractedWorkoutContent
        .split("\n")
        .map((line) => line.trim())
        .filter(Boolean);
      ...
      const filteredWorkoutLines = workoutPlanLines.filter(
        (line) =>
          !line.match(
            /Follow this plan|consistently.*achieve|END-OF-PLAN[\s-]?SUGGESTION|END-OF-PLAN SUGG|END-OF-PLAN|END-OF|END OF|end-of|end of|Closing Recommendation|To achieve your goal|To effectively|SUGGESTION|SUGGEST|RECOMMENDATION|RECOMMEND|CLOSING|CLOSE|FINAL|FINAL NOTE|FINAL SUGGESTION|FINAL RECOMMENDATION/i,
          ),
      );
      ...
      filteredWorkoutLines.forEach((line, index) => {
        ...
        if (exclusionMatch) {
          ...
          [day1, day2, day3].forEach((dayNum) => {
            if (dayNum >= 1 && dayNum <= 7) {
              ...
              console.log(
                `Set Day ${dayNum} as Rest Day due to exclusion message`,
              );
            }
          });
          return;
        ...
        if (singleExclusionMatch) {
          ...
          if (dayNum >= 1 && dayNum <= 7) {
            ...
            console.log(
              `Set Day ${dayNum} as Rest Day due to single exclusion message`,
            );
          }
          return;
        }
        ...
        if (dayMatch) {
          if (currentWorkoutDay && currentExercises.length > 0) {
            const section = workoutSections.find(
              (s) => s.dayNumber === currentWorkoutDay,
            );
            if (section) {
          ...
          currentWorkoutType = dayMatch[2]
            ? dayMatch[2].trim()
            : getWorkoutType(currentWorkoutDay);
          ...
          console.log(
            `Found workout day: ${currentWorkoutDay}, type: ${currentWorkoutType}`,
          );

          if (currentWorkoutDay > 0 && currentWorkoutDay <= 7) {
            let section = workoutSections.find(
              (s) => s.dayNumber === currentWorkoutDay,
            );
            if (!section) {
              section = {
        ...
        if (isProcessingWorkout) {
          ...
          if (isExerciseLine) {
            ...
            if (
              ...
              if (currentWorkoutDay && currentWorkoutDay <= 7) {
                ...
                if (section && section.exercises.length > 0) {
                  const lastExercise =
                    section.exercises[section.exercises.length - 1];
                  // If the last exercise ends with a dash and this line starts with details, combine them
                  if (
                    lastExercise.description.endsWith("—") &&
                    !exercise.includes("—")
                  ) {
                    lastExercise.description =
                      lastExercise.description + " " + exercise;
                    console.log(
                      `Combined exercise: ${lastExercise.description}`,
                    );
                    return;
                  }
                }
            ...
            if (
              ...
              !exercise.includes("min") &&
              !exercise.includes("reps")
            ) {
              console.log(
                `Skipping potentially incomplete exercise: "${exercise}"`,
              );
              return;
            }
            ...
            if (
              ...
              console.log(
                `Buffering potentially incomplete exercise: "${exercise}"`,
              );
              return;
            }
            ...
            if (currentWorkoutDay && currentWorkoutDay <= 7) {
              ...
              if (section) {
                ...
                if (incompleteExerciseBuffer && exercise.includes("—")) {
                  ...
                  console.log(
                    `Combined buffered exercise: "${combinedExercise}"`,
                  );
                ...
                if (!existingExercise && exercise.length > 0) {
                  ...
                  console.log(
                    `Adding exercise to day ${currentWorkoutDay}: ${exercise}`,
                  );
                  ...
                  if (exercise.length > existingExercise.description.length) {
                    ...
                    console.log(
                      `Updated exercise to more complete version: ${exercise}`,
                    );
                  } else if (
                    exercise.includes("—") &&
                    !existingExercise.description.includes("—")
                  ) {
                    // If new exercise has details and existing doesn't, replace it
                    existingExercise.description = exercise;
                    console.log(
                      `Replaced simple exercise with detailed version: ${exercise}`,
                    );
                  } else {
                    console.log(`Skipping duplicate exercise: ${exercise}`);
                  }
                ...
                console.warn(
                  `Cannot add exercise - day ${currentWorkoutDay} not found in sections`,
                );
              }
            } else {
    ...
    workoutSections.forEach((section) => {
      if (
        section.workoutType.toLowerCase().includes("rest") ||
    ...
    const filteredWorkoutSections = workoutSections.filter(
      (section) => !section.shouldRemove,
    );
    ...
    setMealPlansByDay((prevMealPlans) => {
      ...
      if (prevMealPlans.length > 0) {
        ...
        mealPlansByDay.forEach((newDay) => {
          const existingDayIndex = mergedPlans.findIndex(
          ...
          if (existingDayIndex !== -1) {
            ...
            mergedPlans[existingDayIndex] = {
              ...
              meals: newDay.meals.map((newMeal) => {
                const existingMeal = mergedPlans[existingDayIndex].meals.find(
                ...
                if (existingMeal && existingMeal.items.length > 0) {
                  ...
                  return {
                    ...newMeal,
                    items:
                      existingMeal.items.length > 0
                        ? existingMeal.items
                        : newMeal.items,
                    calories:
                      existingMeal.calories > 0
  ...
  const handleSuggestionClick = (suggestion) => {
    ...
    if (userProfileData) {
      ...
      if (userProfileData.activityLevel) {
        ...
        initialFormData.activityLevel =
          activityLevelMap[userProfileData.activityLevel] ||
  ...
  const handleNaturalLanguageInput = async (
    ...
    setProfileFormData({
      ...
      targetWeight:
        extractedData.targetWeight?.value || extractedData.weight || "",
      targetWeightUnit:
        extractedData.targetWeightUnit || extractedData.weightUnit || "kg",
      activityLevel: extractedData.activityLevel || "moderate",
      targetTimeline: extractedData.timelineWeeks
        ? Math.floor(extractedData.timelineWeeks / 4)
        : "3",
    ...
    if (
      ...
      const targetWeight =
        extractedData.targetWeight?.value || extractedData.weight;
      const weightValidation = validateWeightValues(
        currentWeight,
        targetWeight,
      );
  ...
  const getWorkoutIcon = (focusArea) => {
    ...
    if (
      focusArea.toLowerCase().includes("active") ||
      focusArea.toLowerCase().includes("rest")
    )
      return "fa-walking";
    return "fa-dumbbell";
  };
  ...
  const getWorkoutType = (focusArea) => {
    ...
    if (typeof focusArea === "string") {
      ...
      if (focusArea.toLowerCase().includes("cardio"))
      ...
      if (
        focusArea.toLowerCase().includes("active") ||
        focusArea.toLowerCase().includes("recovery")
      )
        return "Flexibility & Recovery";
      if (focusArea.toLowerCase().includes("total")) return "Total Body";
      if (
        focusArea.toLowerCase().includes("core") ||
        focusArea.toLowerCase().includes("abs")
      )
        return "Core & Abs";
    }
  ...
  const storeSubmissionInFirebase = (prompt) => {
    try {
      ...
      try {
        const customerDataScript = document.getElementById(
          "shopify-customer-data",
        );
        if (customerDataScript) {
          const shopifyCustomerData = JSON.parse(
            customerDataScript.textContent,
          );
          if (shopifyCustomerData.id) {
            shopifyCustomerId = shopifyCustomerData.id;
            shopifyCustomerName =
              `${shopifyCustomerData.first_name || ""} ${shopifyCustomerData.last_name || ""}`.trim() ||
              "guest";
      ...
      if (shopifyCustomerId === "guest" && window.customer_data?.id) {
        ...
        shopifyCustomerName =
          `${window.customer_data.first_name || ""} ${window.customer_data.last_name || ""}`.trim() ||
          "guest";
      ...
      if (shopifyCustomerId === "guest" && window.Shopify?.customer?.id) {
        ...
        shopifyCustomerName =
          `${window.Shopify.customer.first_name || ""} ${window.Shopify.customer.last_name || ""}`.trim() ||
          "guest";
  ...
  const isActuallyLoggedIn = () => {
    try {
      ...
      console.log(
        "🔍 AiCoach: AuthContext state - isAuthenticated:",
        isAuthenticated,
        "user:",
        user ? "exists" : "null",
      );
      ...
      if (isAuthenticated && user) {
        console.log(
          "🔐 AiCoach: User authenticated via AuthContext:",
          user.email,
        );
        return true;
      }
      ...
      const verifiedSession = localStorage.getItem("verifiedCustomerSession");
      console.log(
        "🔍 AiCoach: Checking verified session in isActuallyLoggedIn:",
        verifiedSession ? "exists" : "null",
      );
      ...
      if (verifiedSession) {
        try {
          ...
          if (sessionAge < maxAge && sessionData.verified) {
            console.log(
              "🔐 AiCoach: Valid verified customer session found in isActuallyLoggedIn",
            );
            return true;
          } else {
            // Session expired, remove it
            console.log(
              "🔐 AiCoach: Verified customer session expired, removing...",
            );
            localStorage.removeItem("verifiedCustomerSession");
  ...
  useEffect(() => {
    ...
    const checkShopifyCustomer = () => {
      ...
      console.log(
        "🔍 AiCoach: Current state - isAuthenticated:",
        isAuthenticated,
        "user:",
        user ? "exists" : "null",
        "authLoading:",
      ...
      if (isAuthenticated && user) {
        console.log(
          "🔐 AiCoach: User authenticated via AuthContext, setting customer state",
        );
        ...
        if (!isCustomerLoggedIn || !customerData) {
          ...
          console.log(
            "🔐 AiCoach: Customer state already correct, skipping update",
          );
        }
        return;
      }
      ...
      const verifiedSession = localStorage.getItem("verifiedCustomerSession");
      console.log(
        "🔍 AiCoach: Checking verified session:",
        verifiedSession ? "exists" : "null",
      );
      ...
      if (verifiedSession) {
        try {
          ...
          if (sessionAge < maxAge && sessionData.verified) {
            console.log(
              "🔐 AiCoach: Valid verified customer session found, setting customer state",
            );
            ...
            if (!isCustomerLoggedIn || !customerData) {
              ...
              console.log(
                "🔐 AiCoach: Customer state updated from verified session",
              );
            } else {
              console.log(
                "🔐 AiCoach: Customer state already correct, skipping update",
            ...
            localStorage.removeItem("verifiedCustomerSession");
            console.log(
              "🔐 AiCoach: Verified customer session expired, removed",
            );
          }
        } catch (error) {
          console.error("Error parsing verified customer session:", error);
      ...
      if (!isCustomerLoggedIn) {
        ...
        console.log(
          "🔍 AiCoach: Final check - actuallyLoggedIn:",
          actuallyLoggedIn,
        );
        ...
        console.log(
          "🔍 AiCoach: Setting customer state - isCustomerLoggedIn:",
          actuallyLoggedIn,
          "inputLocked:",
          !actuallyLoggedIn,
        );
  ...
  const PlanPDFDocument = ({
    ...
    <Document>
      <Page size={{ width: 600, height: 2000 }} style={pdfStyles.body}>
        {/* EleFit Logo */}
        <Image style={pdfStyles.logo} src="/elefit.jpg" />
                ...
                <View
                  ...
                  style={[
                    ...
                    j % 2 === 0
                      ...
                      : pdfStyles.tableRowOdd,
                  ]}
                >
                  <Text
                    style={{
                      ...pdfStyles.tableCell,
                      ...pdfStyles.mealTypeCell,
                    }}
                  >
                    {meal.type}
                  </Text>
                  <View
                    style={{
                      ...pdfStyles.tableCell,
                      ...pdfStyles.mealItemsCell,
                    }}
                  >
                    {meal.items.map((item, k) => (
                      <Text key={k} style={pdfStyles.mealItem}>
                        - {item.description} ({item.calories} kcal)
                      </Text>
                    ))}
                  </View>
                  <Text
        ...
        {workoutSections &&
          workoutSections.some(
            (section) => section.exercises && section.exercises.length > 0,
          ) && (
            <>
                    ...
                    section.exercises.length === 0 ? (
                      ...
                      <View>
                        {section.exercises.map((ex, j) => (
                          <Text key={j} style={pdfStyles.text}>
                            {j + 1}. {ex.description}
                          </Text>
                        ))}
                      </View>
        ...
        {(mealPlansByDay &&
          mealPlansByDay.length > 0 &&
          endOfPlanSuggestion &&
          endOfPlanSuggestion.trim() !== "") ||
        (workoutSections &&
          workoutSections.some(
            (section) => section.exercises && section.exercises.length > 0,
          ) &&
          workoutPlanSuggestion &&
          workoutPlanSuggestion.trim() !== "") ? (
          <>
  ...
  useEffect(() => {
    if (endOfPlanSuggestion && endOfPlanSuggestion.trim() !== "") {
      ...
      if (!userHasToggledAccordion) {
        ...
        console.log(
          "DEBUG: User has manually controlled accordion, not auto-opening",
        );
      }
      console.log(
        "DEBUG: Suggestions are available:",
        endOfPlanSuggestion.substring(0, 100) + "...",
      );
    }
    ...
    <div className="ai-coach-container">
      ...
      <div className="input-section">
        {isActuallyLoggedIn() && (
          ...
          >
            ...
            {historySecure && (
              ...
              >
                {showConversationHistory ? "Hide" : "Show"} History (
        ...
        <div className="input-wrapper-with-button">
          ...
          >
            <span>
              ...
              >
                <i
                  className={`fas ${copyButtonClicked ? "fa-check" : "fa-lightbulb"}`}
                ></i>
              </button>
            </span>
          </Tooltip>
        ...
        <div className="mobile-prompt-description">
          <div className="tooltip-content-wrapper">
            Click me to create a personalized prompt using your profile
            information (age, weight, height, goals, etc.)
          </div>
        </div>
        ...
        {inputError && (
          <div className="input-error-message">
            <i className="fas fa-exclamation-circle"></i> Please enter your
            fitness goal first
          </div>
        )}
        ...
        {!hidePopularGoals() && (
          <div className="suggestions-container">
            ...
            {currentPopupGoal && goalData[currentPopupGoal] && (
              <div
                className="goal-details-container"
                style={{ borderTopColor: goalData[currentPopupGoal].color }}
              >
                <div className="goal-details-content">
                  ...
                  <p className="goal-details-description">
                    {goalData[currentPopupGoal].description}
                  </p>
                  <form
                    className="goal-details-form"
                    id="goalForm"
                  ...
                  >
                    ...
                    <div className="goal-details-buttons">
                      ...
                      <button
                        type="submit"
                        className="goal-details-button submit"
                      >
                        Continue
                      </button>
                    </div>
        ...
        <div className="loading-video-overlay">
          <div className="loading-video-container">
            ...
            <video className="loading-video" autoPlay loop muted playsInline>
              <source
                src={require("../assets/loading-animation.mp4")}
                type="video/mp4"
              />
      ...
      {historySecure && showConversationHistory && (
        ...
        >
          ...
          >
            ...
            >
              <i
                className={`fas ${isLoadingHistory ? "fa-spinner fa-spin" : "fa-sync-alt"}`}
          ...
          {isLoadingHistory ? (
            ...
            >
              ...
              ></i>
              <p style={{ color: "white" }}>
                No conversation history yet. Start chatting with the AI Coach!
              </p>
              <div
                style={{ marginTop: "10px", fontSize: "12px", color: "white" }}
              >
                Debug: User ID: {user?.uid || "No user"}, History count:{" "}
                {conversationHistory.length}
              </div>
              <button
                onClick={async () => {
                ...
                {conversationHistory.map((conversation, index) => (
                  ...
                  >
                    ...
                    <div style={{ color: "white", textAlign: "center" }}>
                      {conversation.metadata?.profileOption === "yes"
                        ? "Yes"
                        : "No"}
                    </div>
                    <div style={{ color: "white", textAlign: "center" }}>
                      {conversation.metadata?.planData?.hasMealPlan
                        ? "Yes"
                        : "No"}
                    </div>
                    <div style={{ color: "white", textAlign: "center" }}>
                      {conversation.metadata?.planData?.hasWorkoutPlan ||
                      conversation.metadata?.planData?.structuredWorkoutPlan
                        ? "Yes"
                        : "No"}
                    </div>
                  </div>
        ...
        >
          ...
          >
            ...
            <div style={{ marginBottom: "15px", color: "black" }}>
              <strong style={{ color: "black" }}>Timestamp:</strong>{" "}
              {formatTimestamp(selectedConversation.timestamp)}
            </div>

            <div style={{ marginBottom: "15px", color: "black" }}>
              <strong style={{ color: "black" }}>Profile Used:</strong>{" "}
              {selectedConversation.metadata?.profileOption === "yes"
                ? "Yes"
                : "No"}
            </div>

            {selectedConversation.metadata?.profileOption === "yes" &&
              selectedConversation.metadata?.profileDetails && (
                <div style={{ marginBottom: "15px" }}>
                  ...
                  >
                    <div style={{ color: "black" }}>
                      Age: {selectedConversation.metadata.profileDetails.age}
                    </div>
                    <div style={{ color: "black" }}>
                      Gender:{" "}
                      {selectedConversation.metadata.profileDetails.gender}
                    </div>
                    <div style={{ color: "black" }}>
                      Height:{" "}
                      {selectedConversation.metadata.profileDetails.height} cm
                    </div>
                    <div style={{ color: "black" }}>
                      Weight:{" "}
                      {selectedConversation.metadata.profileDetails.weight} kg
                    </div>
                    <div style={{ color: "black" }}>
                      Activity Level:{" "}
                      {
                    ...
                    <div style={{ color: "black" }}>
                      Timeline:{" "}
                      {selectedConversation.metadata.profileDetails.timeline}{" "}
                      months
                    </div>
                  </div>
                </div>
            ...
            {selectedConversation.metadata?.profileOption !== "yes" &&
              selectedConversation.metadata?.freeStylePrompt && (
                <div style={{ marginBottom: "15px" }}>
            ...
            {selectedConversation.metadata?.planData?.structuredWorkoutPlan && (
              <div style={{ marginBottom: "15px" }}>
                ...
                >
                  {selectedConversation.metadata.planData.structuredWorkoutPlan.map(
                    (day, index) => (
                      <div key={index} style={{ marginBottom: "10px" }}>
                        <h4 style={{ color: "black", marginBottom: "5px" }}>
                          Day {day.dayNumber}: {day.workoutType}
                        </h4>
      ...
      {showPlans && (
        ...
        >
          {plansComplete && (
            <div
              className="hide-on-mobile PDF-download-container"
              style={{
                width: "100%",
                marginBottom: "24px",
              }}
          ...
          <div
            className={`plans-grid ${isStreamingMeal || showWorkoutPlanChoice || showWorkoutDaysConfirmation || workoutPlanChoice !== true ? "single-column" : "two-columns"}`}
          >
            <div
              className={`plan-section meal-section ${isStreamingMeal || showWorkoutPlanChoice || showWorkoutDaysConfirmation || workoutPlanChoice !== true ? "" : "slide-left"}`}
            >
              ...
              <div
                className="accordion"
                id="mealAccordion"
                ref={mealAccordionRef}
              >
                {isStreamingMeal ||
                (mealPlansByDay && mealPlansByDay.length > 0) ? (
                  // Show meal days in order, streaming or completed
                  <>
                    {console.log(
                      "Meal plan display - isStreamingMeal:",
                      isStreamingMeal,
                      "mealPlansByDay.length:",
                      mealPlansByDay?.length,
                    )}
                    {mealPlansByDay
                      ...
                      .map((dayData) => (
                        <div
                          key={`streaming-meal-day-${dayData.dayNumber}`}
                          className="accordion-item generated"
                        >
                          <div
                            className="accordion-header"
                            data-meal={`day${dayData.dayNumber}`}
                            onClick={() =>
                              toggleAccordion(`day${dayData.dayNumber}`, "meal")
                            }
                          ...
                          <div
                            ...
                            style={{
                              display:
                                openMealAccordion === `day${dayData.dayNumber}`
                                  ? "block"
                                  : "none",
                              animation:
                                openMealAccordion === `day${dayData.dayNumber}`
                          ...
                          >
                            <div className="nested-accordion">
                              {dayData.meals.map((meal) => (
                                <div
                                  key={`${dayData.dayNumber}-${meal.type}`}
                                ...
                                >
                                  ...
                                  >
                                    <div className="meal-header-content">
                                      <i
                                        className={`fas ${getMealIcon(meal.type)} nested-accordion-icon`}
                                      ></i>
                                      <span
                                        style={{
                                          fontSize: "16px",
                                  ...
                                  >
                                    <ol className="numbered-list" start="1">
                                      {meal.items.map((item, itemIndex) => (
                                        <li
                                          className="ai-coach-numbered-item"
                                          key={`${dayData.dayNumber}-${meal.type}-item-${itemIndex}`}
                                        >
                                          {item.description}{" "}
                                          {item.calories
                                            ? `(${item.calories} kcal)`
                    ...
                    <div key={`day-${dayPlan.dayNumber}`}>
                      <div className="accordion-item">
                        <div
                          ...
                          onClick={() =>
                            toggleAccordion(`day${dayPlan.dayNumber}`, "meal")
                          }
                        >
                          <div className="day-header-content">
                            <i className="fas fa-calendar-day accordion-icon"></i>
                        ...
                        <div
                          ...
                          style={{
                            display:
                              openMealAccordion === `day${dayPlan.dayNumber}`
                                ? "block"
                                : "none",
                            animation:
                              openMealAccordion === `day${dayPlan.dayNumber}`
                        ...
                        >
                          <div className="nested-accordion">
                            {dayPlan.meals.map((meal) => (
                              <div
                                key={`${dayPlan.dayNumber}-${meal.type}`}
                                className="nested-accordion-item"
                              >
                                <div
                                  className="nested-accordion-header"
                                ...
                                >
                                  <ol className="numbered-list" start="1">
                                    {meal.items.map((item, itemIndex) => (
                                      <li
                                        className="ai-coach-numbered-item"
                                        key={`${dayPlan.dayNumber}-${meal.type}-item-${itemIndex}`}
                                      >
                                        {item.description}{" "}
                                        {item.calories
                                          ? `(${item.calories} kcal)`
            ...
            <div
              className={`plan-section workout-section ${isStreamingMeal || showWorkoutPlanChoice || showWorkoutDaysConfirmation || workoutPlanChoice !== true ? "hidden-during-meal" : ""} ${isStreamingMeal ? "locked-section" : ""}`}
            >
              ...
              >
                {isStreamingMeal && !mealPlansByDay.length ? (
                  ...
                  <div className="meal-generation-container">
                    ...
                    <h3 className="meal-generation-title">
                      Crafting Your Perfect Meal Plan
                    </h3>
                    <p className="meal-generation-subtitle">
                      Our AI is analyzing your preferences and creating a
                  ...
                  <div className="no-workout-container">
                    ...
                    <p className="no-workout-subtitle">
                      You documented only meal plan, we recommend you do workout
                      plan also for consistency
                    </p>
                  ...
                  <>
                    {console.log(
                      "Workout plan display - isStreamingWorkout:",
                      isStreamingWorkout,
                    ...
                    {workoutSections
                      ...
                      .map((dayData) => (
                        <div
                          key={`streaming-workout-day-${dayData.dayNumber}`}
                          className="accordion-item generated"
                        >
                          <div
                            className="accordion-header"
                          ...
                          >
                            <div className="day-header-content">
                              <i
                                className={`fas ${getWorkoutIcon(dayData.workoutType)} accordion-icon`}
                              ></i>
                              <span
                                style={{ fontSize: "18px", fontWeight: "600" }}
                              >
                              ...
                              <ol className="numbered-list" start="1">
                                {dayData.exercises.map(
                                  (exercise, exerciseIndex) => (
                                    <li
                                      className="ai-coach-numbered-item"
                                      key={`exercise-${dayData.dayNumber}-${exerciseIndex}`}
                                    >
                                      {exercise.description}
                                    </li>
                                  ),
                    ...
                    <div key={`workout-day-${section.dayNumber}`}>
                      <div className="accordion-item">
                        ...
                        >
                          <div className="day-header-content">
                            <i
                              className={`fas ${getWorkoutIcon(section.workoutType)} accordion-icon`}
                            ></i>
                            <span
                              style={{ fontSize: "18px", fontWeight: "600" }}
                            >
                        ...
                        <div
                          ...
                          style={{
                            display:
                              openWorkoutAccordion === `day${section.dayNumber}`
                                ? "block"
                                : "none",
                            animation:
                              openWorkoutAccordion === `day${section.dayNumber}`
                              ...
                              <ol className="numbered-list" start="1">
                                {section.exercises.map(
                                  (exercise, exerciseIndex) => (
                                    <li
                                      className="ai-coach-numbered-item"
                                      key={`exercise-${section.dayNumber}-${exerciseIndex}`}
                                    >
                                      {exercise.description}
                                    </li>
                                  ),
          ...
          {plansComplete && (
            <div
              className="show-on-mobile PDF-download-container"
              style={{
                width: "100%",
                marginTop: "40px",
                marginBottom: "24px",
          ...
          {(() => {
            const hasPlans =
              (mealPlansByDay && mealPlansByDay.length > 0) ||
              (workoutSections && workoutSections.length > 0 && !noWorkoutPlan);
            ...
            console.log(
              "DEBUG: Suggestions section condition - hasPlans:",
              hasPlans,
              "hasSuggestions:",
              hasSuggestions,
            );
            console.log(
              "DEBUG: mealPlansByDay.length:",
              mealPlansByDay?.length,
              "workoutSections.length:",
              workoutSections?.length,
              "noWorkoutPlan:",
            ...
            console.log(
              "DEBUG: endOfPlanSuggestion length:",
              endOfPlanSuggestion?.length,
              "workoutPlanSuggestion length:",
              workoutPlanSuggestion?.length,
              "personalizedSuggestions length:",
              personalizedSuggestions?.length,
            );
            console.log(
              "DEBUG: openSuggestionsAccordion:",
              openSuggestionsAccordion,
            );
            console.log(
              "DEBUG: Will show suggestions section:",
              hasPlans && hasSuggestions,
            );
            console.log(
              "DEBUG: endOfPlanSuggestion value:",
              endOfPlanSuggestion,
            );
            ...
            <div className="suggestions-section">
              <div
                ...
                style={{
                  ...
                  userSelect: "none",
                  padding: "12px 16px",
                  backgroundColor: openSuggestionsAccordion
                    ? "#f8f9fa"
                    : "#ffffff",
                  borderBottom: openSuggestionsAccordion
                    ? "1px solid #e9ecef"
                    : "none",
                  transition: "all 0.3s ease",
                }}
              >
                ...
                <i
                  className="fas fa-chevron-right accordion-arrow"
                  style={{
                    transform: openSuggestionsAccordion
                      ? "rotate(90deg)"
                      : "rotate(0deg)",
              ...
              <div
                ...
                ref={(el) => {
                  if (el) {
                    console.log(
                      "DEBUG: Suggestions content element display style:",
                      el.style.display,
                    );
                    console.log(
                      "DEBUG: Suggestions content element computed display:",
                      window.getComputedStyle(el).display,
                    );
                    console.log(
                      "DEBUG: Suggestions content element height:",
                      el.offsetHeight,
                    );
                    console.log(
                      "DEBUG: Suggestions content element innerHTML length:",
                      el.innerHTML.length,
                    );
                  }
                }}
                ...
                {isLoadingSuggestions ? (
                  ...
                  <div className="suggestions-text">
                    {(() => {
                      ...
                      const hasMealSuggestion =
                        endOfPlanSuggestion &&
                        endOfPlanSuggestion.trim() !== "";
                      const hasWorkoutSuggestion =
                        workoutPlanSuggestion &&
                        workoutPlanSuggestion.trim() !== "";

                      console.log(
                        "DEBUG: Suggestions content rendering - hasMealSuggestion:",
                        hasMealSuggestion,
                        "hasWorkoutSuggestion:",
                        hasWorkoutSuggestion,
                      );
                      console.log(
                        "DEBUG: Suggestions content rendering - endOfPlanSuggestion:",
                        endOfPlanSuggestion,
                      );
                      console.log(
                        "DEBUG: Suggestions content rendering - workoutPlanSuggestion:",
                        workoutPlanSuggestion,
                      );
                      console.log(
                        "DEBUG: Suggestions content rendering - personalizedSuggestions:",
                        personalizedSuggestions,
                      );
                      console.log(
                        "DEBUG: Suggestions content rendering - isLoadingSuggestions:",
                        isLoadingSuggestions,
                      );
                      ...
                      if (hasMealSuggestion) {
                        suggestions.push(
                          <div
                            key="meal-suggestion"
                            className="explanation-section meal-explanation"
                          >
                      ...
                      if (hasWorkoutSuggestion) {
                        suggestions.push(
                          <div
                            ...
                            className="explanation-section workout-explanation"
                          >
                            <div className="explanation-header">
                      ...
                      if (suggestions.length === 0) {
                        const suggestionsToShow =
                          personalizedSuggestions ||
                          "Follow this plan consistently to achieve your desired results.";
                        const renderedContent = suggestionsToShow
                          .split("\n")
                          .map((line, index) => {
                            ...
                            if (
                              ...
                              const explanation = trimmedLine
                                .replace("MEAL PLAN EXPLANATION:", "")
                                .trim();
                              return (
                                <div
                                  key={index}
                            ...
                            if (
                              ...
                              const explanation = trimmedLine
                                .replace("WORKOUT PLAN EXPLANATION:", "")
                                .trim();
                              return (
                                <div
                                  key={index}
                            ...
                            if (trimmedLine.startsWith("RECOMMENDATION:")) {
                              ...
                              return (
                                <div
                                  key={index}
                                  className="explanation-section recommendation-explanation"
                                >
                                  <div className="explanation-header">
                      ...
                      console.log(
                        "DEBUG: Returning structured suggestions:",
                        suggestions.length,
                      );
                      return suggestions;
                    })()}
      ...
      {showProfileSavePopup && (
        <div className="profile-save-popup">
          <div className="popup-content">
            ...
            <ul>
              {extractedProfileData.height && (
                <li>Height: {extractedProfileData.height} cm</li>
              )}
              {extractedProfileData.weight && (
                <li>Weight: {extractedProfileData.weight} kg</li>
              )}
              {extractedProfileData.age && (
                <li>Age: {extractedProfileData.age}</li>
              )}
              {extractedProfileData.gender && (
                <li>Gender: {extractedProfileData.gender}</li>
              )}
              {extractedProfileData.activityLevel && (
                <li>Activity Level: {extractedProfileData.activityLevel}</li>
              )}
              {extractedProfileData.dietaryRestrictions && (
                <li>
                  Dietary Restrictions:{" "}
                  {extractedProfileData.dietaryRestrictions}
                </li>
              )}
              {extractedProfileData.allergies && (
                <li>Allergies: {extractedProfileData.allergies}</li>
              )}
              {extractedProfileData.healthGoals && (
                <li>Health Goals: {extractedProfileData.healthGoals}</li>
              )}
            </ul>
      ...
      {showProfileChoicePopup && (
        <div className="profile-choice-popup">
          <div className="popup-content">
            ...
            <p>
              You have existing profile information. Would you like to use it
            ...
            >
              {"weight" in changedProfileFields && (
                <div style={{ marginBottom: "8px" }}>
                  ...
                  <span style={{ fontWeight: "500" }}>
                    {changedProfileFields.weight} kg
                  </span>
                </div>
              )}
              {"height" in changedProfileFields && (
                <div style={{ marginBottom: "8px" }}>
                  ...
                  <span style={{ fontWeight: "500" }}>
                    {changedProfileFields.height} cm
                  </span>
                </div>
              )}
              {"age" in changedProfileFields && (
                <div style={{ marginBottom: "8px" }}>
                  ...
                  <span style={{ fontWeight: "500" }}>
                    {changedProfileFields.age} years
                  </span>
                </div>
              )}
              {"gender" in changedProfileFields && (
                <div style={{ marginBottom: "8px" }}>
                  ...
                  <span style={{ fontWeight: "500" }}>
                    {changedProfileFields.gender}
                  </span>
                </div>
              )}
              {"activityLevel" in changedProfileFields && (
                <div style={{ marginBottom: "8px" }}>
                  <span style={{ color: "#666" }}>Activity Level:</span>{" "}
                  <span style={{ fontWeight: "500" }}>
                    {changedProfileFields.activityLevel}
                  </span>
                </div>
              )}
              {changedProfileFields.dietaryRestrictions && (
                <div style={{ marginBottom: "8px" }}>
                  <span style={{ color: "#666" }}>Dietary Restrictions:</span>{" "}
                  <span style={{ fontWeight: "500" }}>
                    {changedProfileFields.dietaryRestrictions}
                  </span>
                </div>
              ...
              {changedProfileFields.allergies && (
                <div style={{ marginBottom: "8px" }}>
                  <span style={{ color: "#666" }}>Allergies:</span>{" "}
                  <span style={{ fontWeight: "500" }}>
                    {changedProfileFields.allergies}
                  </span>
                </div>
              ...
              {changedProfileFields.healthGoals && (
                <div style={{ marginBottom: "8px" }}>
                  <span style={{ color: "#666" }}>Health Goals:</span>{" "}
                  <span style={{ fontWeight: "500" }}>
                    {changedProfileFields.healthGoals}
                  </span>
                </div>
      ...
      {showMissingProfileFieldsPopup && (
        <div
          className="missing-profile-fields-popup"
          style={{
            position: "fixed",
            top: 0,
            left: 0,
        ...
        >
          ...
          >
            ...
            >
              Review and modify your information to generate a personalized
              default prompt.
            </p>
            ...
            <MissingProfileFieldsForm
              ...
              currentProfileData={
                missingProfileFieldsData.currentProfileData || {}
              }
      ...
      {showProfileForm && (
        ...
        >
          ...
          >
            ...
            >
              Please review and complete your details to get personalized
              recommendations.
            </p>
            ...
            >
              ...
              >
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        validateProfileHeight(
                          newHeight,
                          newFormData.heightUnit,
                        );
                      }}
                      onBlur={(e) => {
                        validateProfileHeight(
                          e.target.value,
                          profileFormData.heightUnit,
                        );
                      }}
                      placeholder="Enter height"
                      style={{
                ...
                <div className="form-group">
                  <label htmlFor="profileCurrentWeight">
                    Current Weight <span className="required-asterisk">*</span>
                  </label>
                  <div style={{ display: "flex", gap: "8px", width: "100%" }}>
                    <input
                      type="number"
                      ...
                      onChange={(e) => {
                        ...
                        validateProfileCurrentWeight(
                          newWeight,
                          newFormData.currentWeightUnit,
                        );
                        validateProfileWeights(
                          newWeight,
                          newFormData.targetWeight,
                          newFormData.currentWeightUnit,
                          newFormData.targetWeightUnit,
                        );
                      }}
                      onBlur={(e) => {
                        validateProfileCurrentWeight(
                      ...
                      style={{
                        flex: "1 1 70%",
                        padding: "12px 16px",
                        border: profileCurrentWeightError
                          ? "1px solid #ef4444"
                          : "1px solid #d1d5db",
                        borderRadius: "8px",
                        fontSize: "16px",
              ...
              >
                <div className="form-group">
                  <label htmlFor="profileTargetWeight">
                    Target Weight <span className="required-asterisk">*</span>
                  </label>
                  <div style={{ display: "flex", gap: "8px", width: "100%" }}>
                    <input
                      type="number"
                      ...
                      style={{
                        flex: "1 1 65%",
                        padding: "12px 16px",
                        border: profileTargetWeightError
                          ? "1px solid #ef4444"
                          : "1px solid #d1d5db",
                        borderRadius: "8px",
                        fontSize: "16px",
                ...
                <div className="form-group">
                  <label htmlFor="profileActivityLevel">
                    Activity Level <span className="required-asterisk">*</span>
                  </label>
                  <select
              ...
              <div className="form-row">
                <div className="form-group">
                  <label htmlFor="profileWorkoutDays">
                    Workout Days Per Week{" "}
                    <span className="required-asterisk">*</span>
                  </label>
                  <select
      ...
      {showWorkoutPlanChoice && (
        <div
          className="workout-choice-popup"
          style={{
            position: "fixed",
            top: 0,
            left: 0,
        ...
        >
          ...
          >
            ...
            >
              Your meal plan is ready! Would you also like to generate a
      ...
      {showWorkoutDaysConfirmation && (
        ...
        >
          ...
          >
            ...
            >
              We are generating <strong>{selectedWorkoutDays} days</strong>{" "}
              workout plan based on your input.
            </p>
            <p
              style={{ marginBottom: "30px", color: "#666", fontSize: "14px" }}
            >
            ...
            <div style={{ marginBottom: "30px" }}>
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedWorkoutDays(parseInt(e.target.value))
                }
                style={{
                  padding: "12px 16px",

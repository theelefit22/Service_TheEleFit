...
const AIFitnessCoach = () => {
  ...
  const [showRealTimeResponse, setShowRealTimeResponse] = useState(false);
  const [realTimeLoadingProgress, setRealTimeLoadingProgress] = useState(0);
  const [streamingData, setStreamingData] = useState({ meals: {}, workouts: {} });
  ...
  const extractProfileData = (prompt) => {
    ...
    // Map activity frequency to activity level
  ...
  const generateRealTimePlans = (userDetails) => {
    // Generate immediate sample plans based on user goal
    const goalType = fitnessGoal.toLowerCase();
    let sampleMealPlan = [];
    let sampleWorkoutPlan = [];

    // Generate 7-day sample meal plan
        ...
        meals: [
          {
            ...
            calories: 350,
            items: [
              { description: 'Oatmeal with berries — 80g', calories: 150 },
              { description: 'Greek yogurt — 150g', calories: 120 },
              { description: 'Almonds — 15g', calories: 80 }
            ]
          },
          {
            type: 'Lunch', 
            calories: 450,
            items: [
              { description: 'Grilled chicken breast — 150g', calories: 250 },
              { description: 'Brown rice — 100g', calories: 110 },
              { description: 'Mixed vegetables — 100g', calories: 90 }
            ]
          },
          {
            type: 'Snack',
            calories: 200,
            items: [
              { description: 'Apple — 150g', calories: 80 },
              { description: 'Peanut butter — 20g', calories: 120 }
            ]
          },
          {
            type: 'Dinner',
            calories: 400,
            items: [
              { description: 'Baked salmon — 120g', calories: 200 },
              { description: 'Sweet potato — 150g', calories: 130 },
              { description: 'Steamed broccoli — 100g', calories: 70 }
            ]
          }
        ]
      });
    ...
    // Generate 7-day sample workout plan
    const workoutTypes = goalType.includes('muscle') || goalType.includes('strength') 
      ? ['Upper Body Strength', 'Lower Body Strength', 'Push Day', 'Pull Day', 'Rest Day', 'Full Body', 'Rest Day']
      : goalType.includes('weight') || goalType.includes('cardio')
      ? ['Cardio & Core', 'Full Body Circuit', 'HIIT Training', 'Strength Training', 'Rest Day', 'Active Recovery', 'Rest Day']
      : ['Full Body Workout', 'Cardio Training', 'Strength Training', 'Flexibility & Core', 'Rest Day', 'Circuit Training', 'Rest Day'];

    for (let day = 1; day <= 7; day++) {
      const isRestDay = workoutTypes[day - 1] === 'Rest Day';
      sampleWorkoutPlan.push({
        dayNumber: day,
        workoutType: workoutTypes[day - 1],
        exercises: isRestDay ? [] : [
          { name: 'Push-ups', sets: '3 × 12-15' },
          { name: 'Squats', sets: '3 × 15-20' },
          { name: 'Plank', sets: '3 × 30-60 sec' },
          { name: 'Lunges', sets: '3 × 12 each leg' },
          { name: 'Mountain Climbers', sets: '3 × 20' },
          { name: 'Burpees', sets: '2 × 10' }
        ]
      });
    }

    setMealPlansByDay(sampleMealPlan);
    setWorkoutSections(sampleWorkoutPlan);
    setTimeline(3);
    setWeeklySchedule(7);
    setExpectedResults(`Follow this ${goalType} plan consistently to see results in 3-4 weeks.`);
    setEndOfPlanSuggestion(`Stay consistent with your ${goalType} journey. Results will show with dedication!`);
  };

  const handleStreamingResponse = async (userDetails) => {
    try {
      setIsStreaming(true);
      setStreamingData({ meals: {}, workouts: {} });
      setCurrentStreamStatus('Connecting to server...');
      setShowPlans(true);
      ...
      setCurrentStreamStatus('Generation complete!');
      
    } catch (error) {
      console.error('Streaming error:', error);
      setIsStreaming(false);
      setCurrentStreamStatus('Error occurred during generation');
    }
  };
  ...
  const handleStreamData = (data) => {
    switch (data.type) {
      case 'status':
        setCurrentStreamStatus(data.message);
        break;
      case 'meal_day':
      ...
      case 'workout_day':
        ...
        break;
      case 'complete':
        setEndOfPlanSuggestion(data.suggestion);
        setCurrentStreamStatus('All plans generated successfully!');
        break;
      case 'error':
        setCurrentStreamStatus(`Error: ${data.message}`);
        break;
    }
  };
  ...
  const generatePlanWithProfileChoice = async (useProfile, extractedData) => {
    try {
      ...
      await handleStreamingResponse(userDetails);

      // Check for profile updates after streaming is complete
      ...
      setIsStreaming(false);
      setCurrentStreamStatus('Failed to generate plans');
      throw error;
    }
  };
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      if (auth.currentUser) {
        ...
        setSuccessMessage('You must be logged in to save your profile.');
        setShowSuccessPopup(true);
        setTimeout(() => setShowSuccessPopup(false), 3000);
      }
    } catch (error) {
      console.error('Error saving profile data:', error);
      setSuccessMessage('Failed to save profile data. Please try again.');
      setShowSuccessPopup(true);
      setTimeout(() => setShowSuccessPopup(false), 3000);
    }
    setShowProfileSavePopup(false);
  ...
  const handleNaturalLanguageInput = (prompt) => {
    ...
    setFormData(prevData => ({
      ...
      activityLevel: extractedData.frequency ? 
        (extractedData.frequency <= 2 ? 'light' : 
         extractedData.frequency <= 4 ? 'moderate' : 
         extractedData.frequency <= 6 ? 'active' : 'very active') : 
        prevData.activityLevel
    }));

    // Set fitness goal based on extracted goals
    if (extractedData.goal) {
      ...
      const mappedGoal = goalMapping[extractedData.goal];
  ...
  const handleFormSubmit = async (e) => {
    ...
    // Check if form data has changed from profile
    let hasChanges = false;
    const changedFields = {};
    
    if (userProfileData && userProfileData.age && userProfileData.gender && userProfileData.height && userProfileData.weight && userProfileData.activityLevel) {
      // User has complete profile - check for changes from pre-filled values
      let profileChanges = {};
      
      // Compare age
      const formAge = parseInt(formData.age);
      const profileAge = parseInt(userProfileData.age);
      if (!isNaN(formAge) && !isNaN(profileAge) && formAge !== profileAge) {
        profileChanges.age = formAge;
      }
      
      // Compare gender
      const formGender = String(formData.gender).toLowerCase();
      const profileGender = String(userProfileData.gender || '').toLowerCase();
      if (formGender && formGender !== profileGender) {
        profileChanges.gender = formData.gender;
      }
      
      // Compare height
      const formHeight = parseInt(formData.height);
      const profileHeight = parseInt(userProfileData.height);
      if (!isNaN(formHeight) && !isNaN(profileHeight) && formHeight !== profileHeight) {
        profileChanges.height = formHeight;
      }
      
      // Compare weight
      const formWeight = parseInt(formData.currentWeight);
      const profileWeight = parseInt(userProfileData.weight);
      if (!isNaN(formWeight) && !isNaN(profileWeight) && formWeight !== profileWeight) {
        profileChanges.weight = formWeight;
      }
      
      // Compare activity level
      const formActivity = String(formData.activityLevel).toLowerCase();
      const profileActivity = String(userProfileData.activityLevel || '').toLowerCase();
      if (formActivity && formActivity !== profileActivity) {
        profileChanges.activityLevel = formData.activityLevel;
      }
      
      // Only show update popup if user actually changed something
      if (Object.keys(profileChanges).length > 0) {
        hasChanges = true;
        Object.assign(changedFields, profileChanges);
      }
    } else {
      // No complete profile exists - capture all form data for new profile creation
      hasChanges = true;
      
      // Include all form fields for profile creation
      changedFields.age = parseInt(formData.age) || formData.age;
      changedFields.gender = formData.gender;
      changedFields.height = parseInt(formData.height) || formData.height;
      changedFields.weight = parseInt(formData.currentWeight) || formData.currentWeight;
      changedFields.activityLevel = formData.activityLevel;
    }
    ...
    try {
      ...
      // Show profile update popup if there are changes
      if (hasChanges && Object.keys(changedFields).length > 0) {
        // Check if user has a complete profile
        const hasCompleteProfile = userProfileData && 
      ...
      setIsLoading(false);
      setSuccessMessage(`Failed to fetch recommendations: ${error.message}. Please try again.`);
      setShowSuccessPopup(true);
        ...
        <div className="suggestions-container">
          ...
          <div className="plan-summary">
            <div className="summary-item">
              ...
              <span>Timeline: {timeline} months</span>
            </div>
            <div className="summary-item">
              <i className="fas fa-clock"></i>
              <span>Weekly Schedule: {weeklySchedule} days</span>
            </div>
            <div className="summary-item">
              <i className="fas fa-bullseye"></i>
              <span>Expected Results: {expectedResults}</span>
            </div>
          </div>
          <div className="end-of-plan-suggestion">
            <div className="suggestion-header">
              <i className="fas fa-lightbulb"></i>
              <h3>Recommendation</h3>
            </div>
            <p>{endOfPlanSuggestion}</p>
          </div>
        </div>
      )}
      {(isLoading || showRealTimeResponse) && (
            ...
            <div className="real-time-loading">
              <div className="progress-bar">
                <div 
                  ...
                  style={{ width: `${realTimeLoadingProgress}%` }}
                ></div>
              </div>
              <p className="loading-text">
                Generating your personalized plan... {Math.round(realTimeLoadingProgress)}%
              </p>
              <p className="loading-subtext">
                Your meal and workout plan will appear in {Math.max(0, Math.round((100 - realTimeLoadingProgress) / 20))} seconds
              </p>
            </div>
          ) : (
            <p className="loading-text">
          ...
          <div className="plans-grid">
            <div className="plan-section meal-section">
              ...
              <div className="accordion" id="mealAccordion" ref={mealAccordionRef}>
                {isStreaming ? (
                  // Show streaming meal accordions
                  Array.from({length: 7}, (_, index) => {
                    const dayNumber = index + 1;
                    const dayData = streamingData.meals[dayNumber];
                    const isGenerated = !!dayData;
                    const isGenerating = currentStreamStatus.includes(`Day ${dayNumber} meal`);
                    
                    return (
                      <div key={`streaming-meal-day-${dayNumber}`} className={`accordion-item ${isGenerated ? 'generated' : isGenerating ? 'generating' : 'pending'}`}>
                        <div
                          className="accordion-header"
                          data-meal={`day${dayNumber}`}
                          onClick={() => isGenerated && toggleAccordion(`day${dayNumber}`, 'meal')}
                          style={{ cursor: isGenerated ? 'pointer' : 'default' }}
                        >
                          <div className="day-header-content">
                            <i className={`fas ${isGenerated ? 'fa-calendar-day' : isGenerating ? 'fa-spinner fa-spin' : 'fa-clock'} accordion-icon`}></i>
                            <span style={{fontSize: '18px', fontWeight: '600' }}>Day {dayNumber}</span>
                            {isGenerating && <span className="generating-text">Generating...</span>}
                            {!isGenerated && !isGenerating && <span className="pending-text">Pending</span>}
                          </div>
                          <div className="day-header-right">
                            ...
                            {isGenerated && (
                              <i
                                ...
                                style={{
                                  transform: openMealAccordion === `day${dayNumber}` ? 'rotate(90deg)' : 'rotate(0deg)'
                                }}
                              ></i>
                            )}
                          </div>
                        ...
                        {isGenerated && (
                          <div
                            ...
                            style={{
                              display: openMealAccordion === `day${dayNumber}` ? 'block' : 'none',
                              animation: openMealAccordion === `day${dayNumber}` ? 'slideDown 0.3s ease forwards' : 'none',
                              width: '100%',
                          ...
                          >
                            <div className="nested-accordion">
                              {dayData.meals.map((meal) => (
                                <div key={`${dayNumber}-${meal.type}`} className="nested-accordion-item">
                                  <div 
                                    className="nested-accordion-header"
                                    data-meal-type={meal.type.toLowerCase()}
                                    onClick={() => toggleAccordion(`${dayNumber}-${meal.type}`, 'meal-sub')}
                                  >
                                    <div className="meal-header-content">
                                      <i className={`fas ${getMealIcon(meal.type)} meal-icon`}></i>
                                      ...
                                      <i
                                        ...
                                        style={{
                                          transform: openMealSubAccordion === `${dayNumber}-${meal.type}` ? 'rotate(90deg)' : 'rotate(0deg)'
                                        }}
                                      ></i>
                                    </div>
                                  </div>
                                  <div
                                    ...
                                    style={{
                                      display: openMealSubAccordion === `${dayNumber}-${meal.type}` ? 'block' : 'none',
                                      animation: openMealSubAccordion === `${dayNumber}-${meal.type}` ? 'slideDown 0.3s ease forwards' : 'none'
                                    }}
                                  >
            ...
            <div className="plan-section workout-section">
              ...
              <div className="accordion" id="workoutAccordion" ref={workoutAccordionRef}>
                {isStreaming ? (
                  // Show streaming workout accordions
                  Array.from({length: 7}, (_, index) => {
                    const dayNumber = index + 1;
                    const dayData = streamingData.workouts[dayNumber];
                    const isGenerated = !!dayData;
                    const isGenerating = currentStreamStatus.includes(`Day ${dayNumber} workout`);
                    
                    return (
                      <div key={`streaming-workout-day-${dayNumber}`} className={`accordion-item ${isGenerated ? 'generated' : isGenerating ? 'generating' : 'pending'}`}>
                        <div
                          className="accordion-header"
                          data-workout={`day${dayNumber}`}
                          onClick={() => isGenerated && toggleAccordion(`day${dayNumber}`, 'workout')}
                          style={{ cursor: isGenerated ? 'pointer' : 'default' }}
                        >
                          <div className="day-header-content">
                            <i className={`fas ${isGenerated ? 'fa-calendar-day' : isGenerating ? 'fa-spinner fa-spin' : 'fa-clock'} accordion-icon`}></i>
                          ...
                          <div className="day-header-right">
                            ...
                            {isGenerated && (
                              <i
                                className="fas fa-chevron-right accordion-arrow"
                                style={{
                                  transform: openWorkoutAccordion === `day${dayNumber}` ? 'rotate(90deg)' : 'rotate(0deg)'
                                }}
                              ></i>
                            )}
                          </div>
                        ...
                        {isGenerated && (
                          <div
                            className="accordion-content"
                            style={{
                              display: openWorkoutAccordion === `day${dayNumber}` ? 'block' : 'none',
                              animation: openWorkoutAccordion === `day${dayNumber}` ? 'slideDown 0.3s ease forwards' : 'none',
                              width: '100%',
                              boxSizing: 'border-box'
                            }}
                          >
                            <div className="workout-details">
                              {dayData.exercises.length > 0 ? (
                                ...
                                <p className="rest-day-message">Rest and recovery day. Light stretching or walking recommended.</p>
                              )}
                            </div>
                          </div>
                        )}
                  ...
                  // Show regular workout accordions

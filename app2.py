...
from utils2 import calculate_bmi, calculate_tdee, goal_config, classify_goal_from_text, process_single_day
...
3. Respect user dietary restrictions: {dietary} and allergies: {allergies}. donot inlclude items related to these 
4. Total daily calories must be {calories} kcal ± 20 kcal.
5. Generate all 7 days do not say continue or more or anything else. simply generate everyday
...
"""
    ...
    def event_stream():
        ...
        def process_and_stream_day(day_text, calories):
            print(f"\n--- PROCESSING DAY ---\n{day_text}\n")
            processed_day = process_single_day(day_text, calories)

            # Log comparison GPT vs processed
            print("=== COMPARISON (GPT vs FINAL) ===")
            gpt_lines = [line for line in day_text.splitlines() if "kcal" in line]
            final_lines = [line for line in processed_day.splitlines() if "kcal" in line]
            for g, f in zip(gpt_lines, final_lines):
                print(f"{g.strip()}  ->  {f.strip()}")
            print("=== END COMPARISON ===")
            
            processed_day += "\n"

            # Stream processed day directly
            ...
            if not suggestion_mode and suggestion_phrase in buffer:
                ...
                if day_starts:
                    for i, start in enumerate(day_starts):
                        ...
                        for out in process_and_stream_day(day_text, calories):
                            yield out
                        day_count += 1
...
- Do not generate any markdown or formatting.

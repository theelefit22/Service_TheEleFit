...
def call_openai_grocery_parser(prompt, api_key):
    ...
    try:
        print(f"Calling OpenAI with API key: {api_key[:20]}...")
...
@app.route('/')
def index():
    return "<h2>Combined Fitness Chatbot & PDF Processor Backend is Running</h2><p>Available endpoints: /chat, /check-cache, /process-pdf</p>"
...
def chat_with_gpt():
    ...
    "dietaryRestrictions"
        ...
        missing_fields = [field for field in required_fields if not user_details.get(field)]
        if missing_fields:
            return jsonify({
        ...
        user_profile = {
            "age": user_details.get("dateOfBirth", ""),  # You can calculate age here if needed
            ...
            "activity_level": user_details.get("activityLevel", ""),
            "dietaryRestrictions": [user_details.get("dietaryRestrictions", "")]
        }
        ...
        # # Check Redis cache (skip if force_new is True)
        # if not force_new:
...
WORKOUT_PLAN:
Workout Summary: [Timeline, Weekly Schedule, Expected Results]
...
"""
        ...
        # Store in cache if not forced new
        # if not force_new:
        #     try:

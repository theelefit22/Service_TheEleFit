...
def user_details():
    ...
    if "userDetails" in data:
        ...
        # Handle timeline - only use if explicitly provided and valid
        if timeline_weeks is not None:
            try:
                timeline_weeks = int(timeline_weeks)
                # Validate reasonable range (4 weeks to 2 years)
                if timeline_weeks < 4 or timeline_weeks > 104:
                    timeline_weeks = None
            except (ValueError, TypeError):
                timeline_weeks = None
        
        # If no valid timeline provided, calculate based on weight change goal
        if timeline_weeks is None:
            weight_change = target_weight - weight
            abs_weight_change = abs(weight_change)
            
            if abs_weight_change <= 2:
                timeline_weeks = 8  # 2 months for small changes
            elif abs_weight_change <= 5:
                timeline_weeks = 12  # 3 months for moderate changes
            elif abs_weight_change <= 10:
                timeline_weeks = 16  # 4 months for significant changes
            else:
                timeline_weeks = 24  # 6 months for large changes
            
            print(f"DEBUG: Calculated timeline based on weight change {weight_change}kg: {timeline_weeks} weeks")
